\documentclass[handout]{beamer}

\usepackage[utf8]{inputenc}
\usepackage[frenchb]{babel}
\usepackage{amsmath}
\usepackage{graphicx}
\usepackage{tikz}

\title{Semaine du Numérique}
\author[G. Guisez, J. Hennani, V. Hiairrassary, W. Tassoux]{Grégoire Guisez, Jamal Hennani, Victor Hiairrassary, William Tassoux}
\institute{Polytech' Montpellier, Université Montpellier 2}
\date{2 mai 2012}


\usetheme{Warsaw}

\AtBeginSection[]{
   \begin{frame}
   \begin{center}{\Large Plan }\end{center}
       \tableofcontents[currentsection,hideothersubsections]
   \end{frame} 
}


\begin{document}

\small 

% \begin{frame}[plain] Enleve la décoration de la frame

% Logos à droite collés
%\logo{
%    \includegraphics[height=30px, width=50px]{images/logoUm2.jpg},
%    \includegraphics[height=30px, width=70px]{images/logoPolytech.jpg}
%}

\logo{\includegraphics[height=30px, width=80px]{images/logoPolytech.jpg}}
\setbeamertemplate{sidebar left}
{
    \logo{\includegraphics[height=30px, width=50px]{images/logoUm2.jpg}}
    \vfill
    \rlap{\hskip0.1cm\insertlogo}
    \vskip8pt
}


\begin{frame}
    \titlepage
\end{frame}


\begin{frame}{Introduction}
    \begin{block}{Introduction}
 		 Ce projet vise à mettre en place une partie de la semaine du numérique qui se déroulera lors des années à venir.

		 On peut le décomposer en 2 sous projets:
		\begin{itemize}
			\item Lecteur de carte
			\item Site web
		\end{itemize}
	\end{block}

	\begin{block}{Semaine du Numérique}
	    La semaine du numérique correspond à un évenement ponctuel coordonnée 
    par polytech, en collaboration avec des intervenants extérieurs. Cette 
    derniere sera sanctionné comme tout autre unité d'eseignement par un QCM.
	\end{block}	
\end{frame}


\begin{frame}{Plan}
 	\tableofcontents
\end{frame}

\include{badger}
\include{badger-www}
\include{pdfbox}

\end{document}

