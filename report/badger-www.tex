...
\begin{equation}
    N_{p} = \sum_{i = 0}^{Nb_{inscrs.}} \text{coeff} \times \frac{N_{pmax}}{Nb_{inscrs.}} {,}
    \left\{
    \begin{array}{l l}
        \text{coeff = 1 ssi }Status_{i} = \text{Present}\\
        \text{coeff = -1 ssi }Status_{i} = \text{Absent}\\
    \end{array}
    \right.
\end{equation}

\begin{equation}N_{Q.C.M} = \end{equation}
...
Status_{i} = Status de la i^{ème} inscription\\
\end{array}
\right.
$$
